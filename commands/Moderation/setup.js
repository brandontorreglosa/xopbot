    ...
    async execute(client, message, cmd, args, Discord) {
        ...
        if (message.guild) {
            if (cmd == "setup") {
                ...
                if (target) {
                    const helprole = message.guild.roles.cache.find(role => role.name === 'Requester');
                    ...
                    if (!args[1]) {
                        memberTarget.roles.add(helprole.id);
                        message.channel.send({ content: `*** <@${memberTarget.user.id}> Can Now Send Mail!***` });
                        return
                    }
