...
module.exports = {
    ...
    async execute(client, message, cmd, args, Discord) {
        ...
        const count = 0;const reportssofar = count++; const reportEmbed = new Discord.MessageEmbed().setColor(`${color}`).setTitle('**New Bug Found!**').addField('Author', message.author.toString(), true).addField('Guild', message.guild.name, true).addField('Report', query).addField('Reports So Far:', reportssofar).setThumbnail(message.author.displayAvatarURL({ dynamic: true })).setTimestamp()
