...
module.exports = {
  ...
  async execute(client, message, cmd, args, Discord) {
    ...
    const got = randomNumber + randompercentage;
    ...
    const embed = new Discord.MessageEmbed()
      ...
      .setDescription(`**You Begged And Got From XOPBOT \`${got.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",")}\` Xocoins, Including ${randompercentage} Bonus! 💸**`)
      .setColor(`${color}`)
      .setFooter(`You Have Begged For Over: ${totalbegs.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",")} Times!`)
    message.lineReplyNoMention(embed);
    client.add(message.author.id, got)
  },
};
