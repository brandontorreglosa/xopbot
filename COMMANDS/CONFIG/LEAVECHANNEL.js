...
const goodbyeSchema = require("../../MODELS/LEAVE-CHANNEL");
...
module.exports = {
    ...
    async execute(client, message, cmd, args, Discord) {
        if (!args[0]) {
            const nopr = new Discord.MessageEmbed().setTimestamp().setColor(`${color}`).setAuthor(`${message.author.username}`, message.author.displayAvatarURL({ dynamic: true })).setDescription('**`(prefix)leavechannel <#channel/off>`**')
        ...
        if (message.mentions.channels.first()) {
            ...
            const data2 = await goodbyeSchema.findOne({ GuildID: message.guild.id }); if (data2) {
                await goodbyeSchema.findOneAndRemove({ GuildID: message.guild.id, }); const lvcoff = new Discord.MessageEmbed().setTimestamp().setColor(`${rcolor}`).setAuthor(`${message.author.username}`, message.author.displayAvatarURL({ dynamic: true })).setDescription(`**Leave Channel Has Been \`🔴 Disabled\`!**`)
                return message.lineReplyNoMention({ embed: lvcoff })
            } else if (!data2) {
                const lvcnoset = new Discord.MessageEmbed().setTimestamp().setColor(`${color}`).setAuthor(`${message.author.username}`, message.author.displayAvatarURL({ dynamic: true })).setDescription(`**Leave Channel Not Even Setup Bot!**`)
                message.lineReplyNoMention({ embed: lvcnoset })
            }
        }
    },
};